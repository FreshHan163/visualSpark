...
export default {
  ...
  created () {
    ...
    this.axios.get('/api/getSrcIp0214').then((res) => {
  ...
  methods: {
    getSrcPort51358() {
      return this.axios.get('/api/getSrcIp0214_1016515').then((res) => {
        res.body.forEach((item) => {
          item.bytes = item.bytes / 1000000;
          // item.links = item.links;
        });
    ...
    // },
    getDestIp03() {
      return this.axios.get('/api/getDestIp0214').then((res) => {
        res.body.forEach((item) => {
          item.bytes = item.bytes / 1000000;
          // item.links = item.links;
        });
